...
const MyTaskList = () => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch(`https://namami-infotech.com/TNDMS/src/support/my_ticket.php?EmpId=${EmpId}`);
  ...
  const handleViewTicket = (menuId, ticketId) => {
    navigate(`/training/${menuId}`, {
        state: {
            ticketId: ticketId
        }
    });
                ...
                {paginatedTickets.length > 0 ? (
                  paginatedTickets.map((ticket) => (
                    <TableRow key={ticket.Id} hover>
                      ...
                      <TableCell>
                      <IconButton 
    onClick={() => handleViewTicket(ticket.MenuId, ticket.Id)}
    color="primary"
